...
(defmethod map-btree-node (btree function node)
  (funcall function node)
  (unless (btree-node-leaf-p node)
    (loop for (key . node) across (btree-node-index node) do (map-btree-node btree function node))))

(defun map-btree-nodes (function btree)
    (map-btree-node btree function (btree-root btree)))
